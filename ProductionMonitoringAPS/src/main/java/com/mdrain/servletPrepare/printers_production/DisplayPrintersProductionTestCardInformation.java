...
public class DisplayPrintersProductionTestCardInformation {
	...
	static int defectCollectionCount                          = 0;
	static OrderDefect orders                                 = new OrderDefect();
	static ArrayList<Integer> ordersCollectioFromDefefectList = new ArrayList<Integer>();
	...
	public static void bootstrap(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String[] monthsCollection = {"Януари", "Февруари", "Март", "Април", "Май", "Юни", "Юли", "Август", "Септември", "Октомври", "Ноември", "Декември"};
		...
		int checkMonth = 0;;
		if (req.getParameter("input_test_card_month").equals("Всички")) {
			checkMonth = 0;
		} else {
			month = Integer.parseInt(req.getParameter("input_test_card_month"));
		}
		...
		session.setAttribute("test_card_display_choise_year", year);
		
		if (month == 0) {
			session.setAttribute("test_card_display_choise_month", "Всички");
		} else {
			session.setAttribute("test_card_display_choise_month", month);
		}
		
		String monthForNameChart = "";
		
		if (month == 0) {
			monthForNameChart.equals("");
		} else {
			for (int i = 1; i <= monthsCollection.length; i++) {
				if (month == i) {
					monthForNameChart = " месец " + monthsCollection[i];
				}
			}
		}
		
		session.setAttribute("test_card_display_choise_department", department);
		session.setAttribute("test_card_display_choise_chart_type", typeChart);
		
		session.setAttribute("test_card_display_name_year_month_type", typeChart + " анализ " + " на " + department + " " + year + monthForNameChart);
		
		req.getRequestDispatcher("input_test_card.jsp").forward(req, resp);
		
	}
	
	private static Integer getProductionOrderQuantity(int year, int month, String department,
	...
	private static ArrayList<String> getDefectTypeCollection() {
	...
	private static void sortCollection(ArrayList<Integer> deffectCollectionValue, ArrayList<String> deffectTypeCollection) {
	...
	private static void paretoChart(String testCartDisplayYvalue, String testCartDisplayXvalue, String testCardDisplayTarget,
	...
	private static void mainChart(String testCartDisplayYvalue, String testCartDisplayXvalue, String testCardDisplayTarget,
			ChartGenerator chart, HttpSession session, ArrayList<String> deffectTypeCollection, String typeChart,
	...
	private static String typeChartWithPercentage(String typeChart, ArrayList<Double> deffectCollectionValueDouble,
			double target) {
	...
	private static double getMaxValueFromdeffectCollectionValueDouble(ArrayList<Double> deffectCollectionValueDouble) {
